...
while [ $IDX -lt $CHUNKS ]; do
    for ((i = 0; i < NUM_PARALLEL && IDX < CHUNKS; i++)); do
        ...
        RUN_PREFIX=()
        if [ "$USE_GLX" = "1" ]; then
            RUN_PREFIX=("$ROOT/scripts/runtime/run_glx.sh")
        fi
        echo "Launching job IDX=$IDX on GPU=$GPU_ID; habitat_gpu=$HABITAT_GPU_ID; glx=$USE_GLX"
        CUDA_VISIBLE_DEVICES=$GPU_ID SAVE_VIDEO=$SAVE_VIDEO PYTHONPATH="$ROOT/habitat-lab:${PYTHONPATH:-}" "${RUN_PREFIX[@]}" "$PYTHON_BIN" "$ROOT/run_eval.py" \
            --split-num $CHUNKS \
            --split-id $IDX \
