...
- 当前状态：WP-0数据审计、WP-1统一数据契约和WP-2 Phase 1闭环均已完成。正式8×H100 `phase1_baseline_v3_5103f88`通过B1-ID/B1-GEO/B1-PROBE全部12项gate；但在同一TpT `viz_val`完整序列、同一5,928帧严格协议下，Phase 1身份头E2E IoU≥0.5仅2.59%，冻结Faster R-CNN/OSNet/双运行点融合为46.30%。因此WP-3先执行Phase 2A感知适配。`phase2a_temporal_fusion_v1`因排序退化在val拒绝；v2正确选回与旧模型等价的epoch 0，但重新校准得到tracking margin 0.30，导致val E2E仅13.47%，也已拒绝。v3冻结旧fusion全部既有列/偏置/输出层，仅训练新增时序列，并继承既有train-only双运行点，使epoch 0连排序和阈值都严格等价于冻结基线。两次失败均未运行`viz_val/test_locked`。旧Phase 2 cache继续作废，Habitat closed-loop仍缺。
...
| P0 | NEXT-023 | 训练并准入Phase 2A目标身份时序融合前端 | DEC-039～040、DEC-043～045、EXP-009～011/014～016 | 冻结detector/OSNet的可复现训练配置、checkpoint、train/val/viz指标、失败案例和准入JSON | 进行中；v1排序退化、v2阈值退化均在val拒绝且未运行viz；v3只训练新增列并继承旧双运行点 |
...
| 项目进展记录 | `v23 (2026-09-09)` | 仓库根目录`PROGRESS.md` | 本文件；补记v2失败，v3同时约束legacy权重与既有运行点；closed-loop仍明确阻断 |
