...
RUN_WRAPPER="${RUN_WRAPPER:-$ROOT/scripts/runtime/run_glx.sh}"
...
print_progress() {
  ...
  live_metrics="$("$PYTHON_BIN" "$ROOT/tools/official/live_eval_metrics.py" "$SAVE_PATH" --expected "$EXPECTED_EPISODES")"
...
if [ "$GPU_BURN_DUTY" != "0" ] && [ "$GPU_BURN_DUTY" != "0.0" ]; then
  for gpu in "${GPUS[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES="$gpu" "$PYTHON_BIN" "$ROOT/tools/official/gpu_burn_duty.py" \
      --duty "$GPU_BURN_DUTY" \
