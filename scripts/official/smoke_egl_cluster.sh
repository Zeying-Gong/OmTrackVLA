...
: "${HAB_SIM_TEST_SCENE:=/robot/robot-research-exp-0/user/gzy/habitat-sim-src-egl/data/test_assets/scenes/simple_room.glb}"
: "${SAVE_PATH:=sim_data/eval_smoke/cluster_egl_stt_5steps_$(date +%Y%m%d_%H%M%S)}"
: "${LOG_DIR:=logs/egl_smoke}"
: "${LOG_FILE:=${LOG_DIR}/smoke_egl_cluster_$(date +%Y%m%d_%H%M%S).log}"
...
echo "OMTRACKVLA_NVIDIA_GL_LIBS=${OMTRACKVLA_NVIDIA_GL_LIBS:-}"
echo "OMTRACKVLA_EGL_PRELOAD=${OMTRACKVLA_EGL_PRELOAD:-}"
...
ls -l /etc/glvnd/egl_vendor.d 2>/dev/null || true
echo
echo "[run_egl.sh]"
ls -l $ROOT/scripts/runtime/run_egl.sh

echo
echo "================================================"
echo "Step 1/2: minimal Habitat-Sim EGL context test"
echo "================================================"

$ROOT/scripts/runtime/run_egl.sh "${PYTHON_BIN}" - <<'PY'
import os
import habitat_sim
import magnum
...
$ROOT/scripts/runtime/run_egl.sh "${PYTHON_BIN}" run_eval.py \
