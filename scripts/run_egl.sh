...
HAB_SIM_EGL_ROOT="${OMTRACKVLA_HAB_SIM_EGL_ROOT:-}"
NVIDIA_GL_LIBS="${OMTRACKVLA_NVIDIA_GL_LIBS:-system}"
...
if [[ -n "$HAB_SIM_EGL_ROOT" && -d "$HAB_SIM_EGL_ROOT" ]]; then
  export PYTHONPATH="${HAB_SIM_EGL_ROOT}/build/deps/magnum-bindings/src/python:${HAB_SIM_EGL_ROOT}/src_python:${REPO_ROOT}/habitat-lab:${REPO_ROOT}:${PYTHONPATH:-}"
else
  export PYTHONPATH="${REPO_ROOT}/habitat-lab:${REPO_ROOT}:${PYTHONPATH:-}"
fi

echo "[run_egl] HAB_SIM_EGL_ROOT=${HAB_SIM_EGL_ROOT:-conda-package}"
