...
INITIAL="$ROOT/outputs/training/v2_003_se2_visibility/checkpoints/best.ckpt"
PREFLIGHT="$ROOT/outputs/training/v2_004_se2_visibility_balanced_preflight"
OUTPUT="$ROOT/outputs/training/v2_004_se2_visibility_balanced"
EVAL="$ROOT/outputs/evaluation/v2_004_se2_visibility_balanced/stt/index2"
...
if [[ ! -f "$PREFLIGHT/TRAINING_COMPLETE.json" ]]; then
  "$PYTHON" -m torch.distributed.run \
    --standalone --nproc_per_node=7 --master_port=29662 \
...
if [[ ! -f "$OUTPUT/TRAINING_COMPLETE.json" ]]; then
  "$PYTHON" -m torch.distributed.run \
    --standalone --nproc_per_node=7 --master_port=29663 \
    --module omtrackvla.training.end_to_end_v2 \
    --config "$CONFIG" \
    --output-dir "$OUTPUT" \
    --initial-checkpoint "$INITIAL"
