...
case "$RENDER_BACKEND" in
  egl)
    RENDER_RUNNER="$ROOT/scripts/runtime/run_egl.sh"
    ;;
  xvfb)
    RENDER_RUNNER="$ROOT/scripts/runtime/run_xvfb.sh"
    ;;
  *)
...
"$PYTHON_BIN" "$ROOT/tools/modular/summarize_oracle_modular.py" "$OUTPUT_ROOT" --clean-invalid-only
...
if [[ "$GPU_BURN_DUTY" != "0" && "$GPU_BURN_DUTY" != "0.0" ]]; then
  for gpu in "${PHYSICAL_GPUS[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES="$gpu" "$PYTHON_BIN" -u "$ROOT/tools/official/gpu_burn_duty.py" \
      --duty "$GPU_BURN_DUTY" \
...
for task in "${TASK_ARRAY[@]}"; do
  for split in "${SPLIT_ARRAY[@]}"; do
    ...
    progress_baseline="$(
      "$PYTHON_BIN" "$ROOT/tools/modular/monitor_oracle_progress.py" "${PROGRESS_ARGS[@]}" --count-only
    )"
    "$PYTHON_BIN" -u "$ROOT/tools/modular/monitor_oracle_progress.py" \
      "${PROGRESS_ARGS[@]}" \
    ...
    "$PYTHON_BIN" "$ROOT/tools/modular/monitor_oracle_progress.py" \
      "${PROGRESS_ARGS[@]}" \
      --baseline "$progress_baseline" \
      --started-at "$progress_started_at" \
      --once
    combo=$((combo + 1))
    "$PYTHON_BIN" "$ROOT/tools/modular/summarize_oracle_modular.py" "$OUTPUT_ROOT"
  done
done
...
"$PYTHON_BIN" "$ROOT/tools/modular/summarize_oracle_modular.py" "$OUTPUT_ROOT" "${summary_args[@]}"
