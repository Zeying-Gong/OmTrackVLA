...
if [[ ! -x "$PYTHON_BIN" ]]; then
  echo "[oracle-8gpu] ERROR: Python not found: $PYTHON_BIN" >&2
  exit 2
fi
...
"$PYTHON_BIN" "$ROOT/scripts/summarize_oracle_modular.py" "$OUTPUT_ROOT" --clean-invalid-only
...
if [[ "$GPU_BURN_DUTY" != "0" && "$GPU_BURN_DUTY" != "0.0" ]]; then
  for gpu in "${PHYSICAL_GPUS[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES="$gpu" "$PYTHON_BIN" -u "$ROOT/scripts/gpu_burn_duty.py" \
      --duty "$GPU_BURN_DUTY" \
...
for task in "${TASK_ARRAY[@]}"; do
  for split in "${SPLIT_ARRAY[@]}"; do
    ...
    progress_baseline="$(
      "$PYTHON_BIN" "$ROOT/scripts/monitor_oracle_progress.py" "${PROGRESS_ARGS[@]}" --count-only
    )"
    "$PYTHON_BIN" -u "$ROOT/scripts/monitor_oracle_progress.py" \
      "${PROGRESS_ARGS[@]}" \
    ...
    "$PYTHON_BIN" "$ROOT/scripts/monitor_oracle_progress.py" \
      "${PROGRESS_ARGS[@]}" \
      --baseline "$progress_baseline" \
      --started-at "$progress_started_at" \
      --once
...
"$PYTHON_BIN" "$ROOT/scripts/summarize_oracle_modular.py" "$OUTPUT_ROOT" "${summary_args[@]}"
