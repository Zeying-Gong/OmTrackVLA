#!/usr/bin/env python3
"""Controlled 64-step development pilot using complete variable-length prefixes.
...
def clean_training_objective(outputs, batch, weights, loss_fn, *, multiplier=8.):
    """Only scale zero-waypoint supervision for the explicit safe-stop mode.

    Four modes still share the original batch denominator. Auxiliary heads and
    the other three modes retain their original gradient weights. Validation
    continues to report the original unweighted metrics for direct comparison.
    """
    require(multiplier == 8., 'this experiment freezes the safe-stop multiplier at eight')
    modes = batch['mode_index']
    require(modes.ndim == 1 and modes.numel() == 4 and sorted(modes.tolist()) == [0, 1, 2, 3],
            'the controlled clean batch requires one sample per mode')
    selected = modes == 3
    require(tuple(outputs['waypoints'].shape) == (4, 2, 8, 2)
            and tuple(batch['waypoint_mask'].shape) == (4, 2, 8)
            and batch['waypoint_mask'].all().item()
            and ('supervision_mask' not in batch or batch['supervision_mask'].all().item()),
            'controlled clean weighting requires equally supervised two-call sequences')
    require((batch['stop_target'][selected] == 1).all().item()
            and (batch['target_waypoints'][selected] == 0).all().item()
            and (batch['visual_initialization_valid'][selected] == 0).all().item()
            and (batch['uwb_valid'][selected] == 0).all().item(),
            'safe-stop supervision is not consistent with the explicit no-target mode')
    require((batch['binding_valid'][selected] == 0).all().item()
            and (batch['rgb_valid'][selected] == 1).all().item(),
            'safe-stop experiment expects available scene RGB without target binding')
    total, _ = loss_fn(outputs, batch, weights)
    stop_waypoint, _ = loss_fn(
        {'waypoints': outputs['waypoints'][selected]},
        {key: batch[key][selected] for key in ('target_waypoints', 'waypoint_mask')},
        {'waypoint': 1.})
    fraction = float(selected.sum().item()) / modes.numel()
    return total + (multiplier - 1.) * fraction * float(weights.get('waypoint', 0.)) * stop_waypoint
...
def effective_configuration(config, base, *, output_dir):
    ...
    require(config.get('stage') == 'recovery_sequence_variable_prefix_development_v4', 'wrong experiment stage')
    manifest_value = config.get('recovery_manifest')
    require(isinstance(manifest_value, str) and bool(manifest_value.strip())
            and not manifest_value.startswith('__ROOT_MUST_FREEZE'),
            'root must freeze an admitted recovery manifest before running this template')
    readiness_value = config.get('training_readiness')
    readiness_sha = config.get('training_readiness_sha256')
    require(isinstance(readiness_value, str) and bool(readiness_value.strip())
            and not readiness_value.startswith('__ROOT_MUST_FREEZE')
            and isinstance(readiness_sha, str) and len(readiness_sha) == 64
            and all(char in '0123456789abcdef' for char in readiness_sha),
            'root must freeze the training readiness path and SHA-256 before running this template')
    train = config['training']
    require('burn_in_steps' not in train
            and train.get('burn_in_rule') == 'max(0, sequence_length - 4)',
            'v4 requires the complete real prefix and per-sample dynamic burn-in')
    for key, expected in {'maximum_steps': 64, 'learning_policy_calls': 4,
    ...
    require(train.get('safe_stop_waypoint_multiplier') == 8., 'safe-stop experiment multiplier changed')
...
def verify_training_readiness(repository, config, manifest_path, hash_file):
    """Check the frozen data-coverage gate before GPU setup or source hashing."""
    path = resolve(repository, config['training_readiness'])
    payload = path.read_bytes()
    actual_sha = hashlib.sha256(payload).hexdigest()
    require(actual_sha == config['training_readiness_sha256'], 'training readiness SHA-256 mismatch')
    readiness = json.loads(payload)
    require(readiness.get('training_ready') is True
            and readiness.get('status') == 'ready_for_development_pilot'
            and readiness.get('validation_unchanged') is True,
            'training readiness gate is not satisfied; do not start an empty or unadmitted pilot')
    bound_manifest = readiness.get('manifest', {})
    require(isinstance(bound_manifest, dict) and isinstance(bound_manifest.get('path'), str),
            'readiness must bind the admitted manifest path and hash')
    require(resolve(repository, bound_manifest['path']) == manifest_path.resolve(strict=True),
            'readiness belongs to another manifest path')
    require(bound_manifest.get('sha256') == hash_file(manifest_path),
            'readiness manifest hash differs from the actual manifest')
    return path, actual_sha, readiness
...
def recovery_sequence_metadata(identity, burn_in_steps):
    return {'manifest_index': identity.manifest_index, 'sample_id': identity.sample_id,
            'sample_path': str(identity.sample_path), 'partition_role': identity.partition_role,
            'anchor_policy_call_index': identity.anchor_policy_call_index,
            'sequence_length': identity.sequence_length, 'history_size': identity.history_size,
            'burn_in_steps': burn_in_steps}


def prepare_recovery(item, torch, device, *, for_optimizer):
    """Keep all observations; the admitted loader alone slices real labels.

    The caller must use the returned burn-in for the central sequence helper.
    Every item starts a fresh unroll; no state or padded calls cross samples.
    """
    require(type(for_optimizer) is bool, 'optimizer intent must be an explicit boolean')
    prepared = item.as_batch(for_optimizer=for_optimizer)
    steps = prepared.identity.sequence_length
    require(prepared.batch['ego_rgb'].shape[:2] == (1, steps), 'complete recovery prefix shape changed')
    require(prepared.burn_in_steps == max(0, steps - 4), 'recovery burn-in rule changed')
    require(prepared.labels['supervision_mask'].shape == (1, min(4, steps)),
            'recovery labels do not align with the learning suffix')
    return (move_batch(prepared.batch, torch, device), move_batch(prepared.labels, torch, device),
            prepared.burn_in_steps, recovery_sequence_metadata(prepared.identity, prepared.burn_in_steps))
...
def main(argv=None):
    ...
    readiness_path, readiness_sha, readiness = verify_training_readiness(
        repository, config, manifest_path, file_hash)
    ...
    loader_options = dict(artifact_root=repository, image_height=architecture.image_height,
                          image_width=architecture.image_width)
    recovery_train = VariableRecoverySequences(manifest_path, partition_role='train', for_training=True, **loader_options)
    recovery_val = VariableRecoverySequences(manifest_path, partition_role='val', for_training=False, **loader_options)
    require([item.sample_path for item in recovery_train.identities] == train_paths
            and [item.sample_path for item in recovery_val.identities] == val_paths,
            'variable loader changed the verified manifest sample order')
    require(recovery_train.manifest_sha256 == recovery_val.manifest_sha256
            == readiness['manifest']['sha256'] == file_hash(manifest_path),
            'admitted manifest changed after the readiness check')
    data_hashes = {'recovery_manifest': file_hash(manifest_path), 'parent_checkpoint': PARENT_SHA,
                   'training_readiness': readiness_sha,
    ...
    with RunDirectory(output_dir, contract, resume=args.resume_from is not None) as run:
        ...
        def evaluate():
            ...
            try:
                with torch.inference_mode(), autocast():
                    ...
                    for index in range(len(recovery_val)):
                        batch, labels, burn_in, identity = prepare_recovery(
                            recovery_val[index], torch, device, for_optimizer=False)
                        require(identity['manifest_index'] == index, 'validation manifest index drifted')
                        outputs = wrapped(**sequence_model_inputs(batch), burn_in_steps=burn_in)
                        metrics = prediction_metrics(outputs, labels, recovery_weights, torch, phase3_sequence_loss)
                        recovery_metrics.append(metrics)
                        recovery_sequence_records.append(dict(identity, metrics=metrics))
            ...
            return {'clean': {'aggregate': average_metrics([m for values in by_mode.values() for m in values]),
                    ...
                    'recovery_val_sequences': recovery_sequence_records}
        ...
        if resume is None:
            atomic_json(output_dir / 'run_manifest.json', {
                ...
                'training_readiness': {'path': str(readiness_path), 'sha256': readiness_sha},
                'recovery_train_paths': [str(path) for path in train_paths], 'recovery_val_paths': [str(path) for path in val_paths],
                'recovery_train_sequences': [recovery_sequence_metadata(item, max(0, item.sequence_length - 4))
                                             for item in recovery_train.identities],
                'recovery_val_sequences': [recovery_sequence_metadata(item, max(0, item.sequence_length - 4))
                                           for item in recovery_val.identities],
        ...
        for step in range(start_step, 64):
            ...
            def clean_forward():
                ...
                with autocast():
                    outputs = wrapped(**sequence_model_inputs(batch), burn_in_steps=0)
                    loss = clean_training_objective(outputs, batch, clean_weights, phase3_sequence_loss,
                        multiplier=training['safe_stop_waypoint_multiplier'])
            ...
            def recovery_forward():
                batch, labels, burn_in, identity = prepare_recovery(
                    recovery_train.optimizer_item(recovery_index), torch, device, for_optimizer=True)
                require(identity['manifest_index'] == recovery_index, 'training manifest index drifted')
                recovery_sequence_log.update(identity)
                with autocast():
                    outputs = wrapped(**sequence_model_inputs(batch), burn_in_steps=burn_in)
            ...
            record = {'global_step': global_step, 'clean_indices': clean_indices, 'recovery_index': recovery_index,
                      ...
                      'recovery_sequence': recovery_sequence_log,
