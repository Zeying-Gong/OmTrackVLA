#!/usr/bin/env python3
"""Collect candidate recovery sequences with measured simulator time.

The saved policy actions are replayed unchanged. Poses and world times are
recorded separately for audit; only the label-side expert consumes privileged
geometry. Candidate targets use observed time, and formal eligibility is false.
The legacy expert quality audit is retained as an additional diagnostic.
"""
...
from recovery_sequence_export_v2 import audit_state, write_candidate_sequence
...
def arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--original-sample", type=Path, help="Optional matching legacy sample reference; never admission")
...
def main() -> int:
    ...
    if result.get("split") != "train":
        raise ValueError("recovery sequence candidate collection accepts EVT train only")
    ...
    expert_states: list[dict[str, object]] = []
    waypoints: list[list[float]] = []
    phase3_sample: dict[str, object] | None = None
    def reset_and_replay(
        replay_env: Any,
    ) -> tuple[Any, np.ndarray, list[tuple[int, np.ndarray]], list[dict[str, object]]]:
        replay_env.reset()
        ...
        replay_states = [audit_state(replay_env, 0)]
        for replay_step, action in enumerate(
            ...
            replay_states.append(audit_state(replay_env, replay_step))
        return replay_observations, replay_initial_rgb, replay_history, replay_states
    ...
    with habitat.TrackEnv(config=config, dataset=dataset) as env:
        observations, initial_rgb, replay_rgb, replay_states = reset_and_replay(env)
        ...
        for expert_step in range(args.expert_horizon_steps + 1):
            expert_states.append(audit_state(env, args.checkpoint_step + expert_step))
            ...
            expert_records.append({
                ...
                "world_time_s_before": expert_states[-1]["world_time_s"],
                "robot_world_xyz_m_before": expert_states[-1]["robot_world_xyz_m"],
                "target_world_xyz_m_before": expert_states[-1]["target_world_xyz_m"],
            ...
            after_state = audit_state(env, args.checkpoint_step + expert_step + 1)
            expert_records[-1].update({
                "world_time_s_after": after_state["world_time_s"],
                "robot_world_xyz_m_after": after_state["robot_world_xyz_m"],
                "target_world_xyz_m_after": after_state["target_world_xyz_m"],
            })
            metrics = env.get_metrics()
            collisions = max(
                collisions, float(metrics.get("human_collision", 0.0) or 0.0)
            )
    ...
    payload = {
        ...
        "stage": "next007_recovery_sequence_probe_v2",
        ...
        "label_boundary": {
            "gt_pose_used_only_in_audit_and_supervision": True,
            "replay_actions_never_use_audit_pose": True,
        ...
        "expert": {
            ...
            "legacy_step_stride_offsets_seconds_observed": [float(expert_states[value]["world_time_s"]) - float(expert_states[0]["world_time_s"]) for value in offsets],
            "legacy_step_stride_waypoints_are_not_v2_training_targets": True,
    ...
    payload["actual_time_audit"] = {
        "world_time_reader": "env.sim.get_world_time",
        "prefix_states": replay_states,
        "expert_states": expert_states,
        "fixed_frequency_assumed": False,
        "evt_bench_train_used_for_waypoint_supervision": True,
        "legacy_passed_is_not_formal_admission": True,
    }
    # Persist the raw replay/expert audit even if candidate time coverage rejects.
    _atomic_json(args.output, payload)
    if passed and args.phase3_sample_dir is not None:
        intrinsics, camera_from_base = habitat_camera_calibration(
            initial_rgb.shape, output_height=args.model_image_height,
            output_width=args.model_image_width, hfov_deg=90.0,
        )
        manifest_path, manifest_sha256 = write_candidate_sequence(
            directory=args.phase3_sample_dir,
            source_rollout=result_path,
            report_output=args.output.expanduser().resolve(strict=False),
            result=result, initial_rgb=initial_rgb,
            prefix_frames=replay_rgb, prefix_states=replay_states,
            initial_bbox_xyxy_norm=normalized_bbox(initialization["bbox_xyxy"], initial_rgb.shape),
            camera_intrinsics=intrinsics, camera_from_base=camera_from_base,
            expert_states=expert_states, expert_actions=expert_records,
            replay_actions=actions[:args.checkpoint_step],
            anchor_position=anchor_position, forward_axis=forward_axis, left_axis=left_axis,
            checkpoint_step=args.checkpoint_step, history_size=args.history_size,
            legacy_quality_passed=passed, original_sample=args.original_sample,
        )
        phase3_sample = {
            "manifest": str(manifest_path), "manifest_sha256": manifest_sha256,
            "prefix_environment_steps": [item[0] for item in replay_rgb],
            "formal_training_eligible": False,
            "stage": "phase3_recovery_sequence_candidate_v2",
        }
        payload["phase3_recovery_sequence_candidate"] = phase3_sample
    _atomic_json(args.output, payload)
    print(json.dumps({
