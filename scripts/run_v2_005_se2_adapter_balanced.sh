...
CONFIG="$ROOT/configs/phases/phase2_end_to_end_v2_se2_adapter_balanced.yaml"
INITIAL="$ROOT/outputs/training/v2_004_se2_visibility_balanced/checkpoints/best.ckpt"
PREFLIGHT="$ROOT/outputs/training/v2_005_se2_adapter_balanced_preflight"
OUTPUT="$ROOT/outputs/training/v2_005_se2_adapter_balanced"
EVAL_ROOT="$ROOT/outputs/evaluation/v2_005_se2_adapter_balanced/stt/index2"
...
if [[ ! -f "$PREFLIGHT/TRAINING_COMPLETE.json" ]]; then
  "$PYTHON" -m torch.distributed.run \
    --standalone --nproc_per_node=7 --master_port=29672 \
...
if [[ ! -f "$OUTPUT/TRAINING_COMPLETE.json" ]]; then
  "$PYTHON" -m torch.distributed.run \
    --standalone --nproc_per_node=7 --master_port=29673 \
    --module omtrackvla.training.end_to_end_v2 \
    --config "$CONFIG" \
    --output-dir "$OUTPUT" \
    --initial-checkpoint "$INITIAL"
...
run_eval() {
  local name="$1"
  shift
  local destination="$EVAL_ROOT/$name"
  mkdir -p "$destination"
...
write_status closed_loop_gated running 8192
run_eval gated
write_status closed_loop_no_failsafe running 8192
run_eval no_failsafe --disable-visual-failsafe
